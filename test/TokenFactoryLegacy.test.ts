...
import type { TokenFactoryLegacy, CurrencyTokenLegacy } from "../typechain-types/contracts/TokenFactoryLegacy";

describe("TokenFactoryLegacy & CurrencyTokenLegacy", function () {
	async function deployFactory() {
		const [admin, deployer, owner, user1, user2] = await ethers.getSigners();

		const Factory = await ethers.getContractFactory("TokenFactoryLegacy");
		const factory = (await upgrades.deployProxy(Factory, [admin.address, deployer.address], {
			initializer: "initialize"
		})) as unknown as TokenFactoryLegacy;
	...
	async function deployTokenFromFactory() {
		...
		const token = (await ethers.getContractAt("CurrencyTokenLegacy", tokenAddress)) as unknown as CurrencyTokenLegacy;
	...
	describe("TokenFactoryLegacy deployment", function () {
	...
	describe("Deployed CurrencyTokenLegacy basic ERC20 behaviour", function () {
